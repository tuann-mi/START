...
export default function Addresses() {
    ...
    return (
        <div id="addresses-page-container" className="flex w-screen h-full max-w-full bg-white dark:bg-gray-900 transition-colors duration-300">
            <Sidebar />
            <div id="addresses-content-container" className="flex-grow p-6 w-96">
                <h1 className="text-3xl mb-4 text-gray-900 dark:text-white font-bold">Addresses</h1>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-black">Date Last Sampled</TableHead>
                                ...
                                {data.map((item, index) => (
                                    <TableRow key={index}>
                                        <TableCell>
                                            ...
                                            >
                                                <SheetTrigger className="text-som-primary underline" onClick={() => setOpenAddress(item.sampled_address_clean)}>{item.sampled_address_clean}</SheetTrigger>
                                                <SheetContent>
                                                    <SheetHeader>
                                                        <SheetTitle>{item.sampled_address_clean}</SheetTitle>
                                                        <SheetDescription>
                                                            <p>Site Name: <Link href={`/dashboard/sites?siteName=${encodeURIComponent(item.site_name)}`} className="text-som-primary underline">{item.site_name}</Link></p>
