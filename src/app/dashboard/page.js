...
export default function Dashboard() {
  ...
  return (
    <div id="dashboard-page" className="flex w-screen h-full max-w-7xl">
          ...
          <div id="dashboard-content-container" className="flex-grow p-6 w-96">
            ...
            {selectedTables.includes('general') && (
              <div id="general-info-container" className="border-2 border-gray-500 rounded-md p-4">
                <h1 className="text-xl mb-4 text-gray-900 dark:text-white">General Info</h1>
                <div className="flex flex-row justify-between items-center rounded-md space-x-4 overflow-x-auto pb-4">
                  <div className="flex flex-col rounded-md p-4 bg-gray-400 min-w-60">
                      <p className="text-xl text-gray-900 dark:text-white font-bold underline min-h-16">Active Sites</p>
                      <span className="font-bold text-12xl text-gray-900 dark:text-white">##</span>
                    </div>
                    <div className="flex flex-col rounded-md p-4 bg-gray-400 min-w-60">
                      <p className="text-xl text-gray-900 dark:text-white font-bold underline min-h-16">Mailings Sent</p>
                      <span className="font-bold text-12xl text-gray-900 dark:text-white">##</span>
                    </div>
                    <div className="flex flex-col rounded-md p-4 bg-gray-400 min-w-60">
                      <p className="text-xl text-gray-900 dark:text-white font-bold underline min-h-16">Homes Sampled</p>
                      <span className="font-bold text-12xl text-gray-900 dark:text-white">##</span>
                    </div>
                    <div className="flex flex-col rounded-md p-4 bg-gray-400 min-w-60">
                      <p className="text-xl text-gray-900 dark:text-white font-bold underline min-h-16">Samples Processed</p>
                      <span className="font-bold text-12xl text-gray-900 dark:text-white">##</span>
                    </div>
                  </div>
              </div>
            ...
            <div id="dashboard-tables-container" className="flex w-full">
              <div id="dashboard-tables" className="flex-grow py-4 mt-4 h-full overflow-x-auto">
                {selectedTables.includes('workOrders') && (
                  <div className="table-container max-h-96 overflow-x-scroll mb-8 border-2 border-gray-500 rounded-md p-4">
                    <h1 className="text-xl mb-4 text-gray-900 dark:text-white">Work Orders</h1>
                    <DataTable data={sortedData(workOrdersData)} columns={columns.workOrders} />
                  </div>
                )}
                {selectedTables.includes('targetActual') && (
                  <div className="table-container max-h-96 overflow-x-scroll mb-8 border-2 border-gray-500 rounded-md p-4">
                    <h1 className="text-xl mb-4 text-gray-900 dark:text-white">Target vs Actual</h1>
                    <DataTable data={sortedData(targetActualData)} columns={columns.targetActual} />
                  </div>
                )}
                {selectedTables.includes('samplingStatus') && (
                  <div className="table-container max-h-96 overflow-x-scroll mb-8 border-2 border-gray-500 rounded-md p-4">
                    <h1 className="text-xl mb-4 text-gray-900 dark:text-white">Sampling Round Status</h1>
                    <DataTable data={sortedData(samplingStatusData)} columns={columns.samplingStatus} />
                  </div>
                )}
                {selectedTables.includes('callTracking') && (
                  <div className="table-container max-h-96 overflow-x-scroll mb-8 border-2 border-gray-500 rounded-md p-4">
                    <h1 className="text-xl mb-4 text-gray-900 dark:text-white">Call Tracking</h1>
                    <DataTable data={sortedData(samplingStatusData)} columns={columns.samplingStatus} />
                  </div>
                )}
              </div>
