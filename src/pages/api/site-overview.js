...
export default async function handler(req, res) {
  ...
  try {
    console.log('(SITE OVERVIEW) Connecting to the database...');
    await client.connect();
    console.log('(SITE OVERVIEW) Connected to the database.');
    ...
    `);
    console.log('(SITE OVERVIEW) Full query executed successfully:', result.rows.slice(0, 5));
    res.status(200).json(result.rows);
  } catch (error) {
    console.error('(SITE OVERVIEW) Error executing query:', error);
    res.status(500).json({ error: 'Failed to fetch data', details: error.message });
  } finally {
    await client.end();
    console.log('(SITE OVERVIEW) Disconnected from the database.');
  }
}